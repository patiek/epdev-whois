...
class EP_Dev_Whois_Display
{
	...
	function displayPage()
	{
		...
		// IDN support (charset needs to be UTF-8)
		if ($this->CORE->CONFIG->SCRIPT['translate_idn'] && !headers_sent())
			header("Content-Type: text/html; charset=UTF-8");

		// attempt to include custom header
		if ($this->CORE->CONFIG->SCRIPT['custom_header'] != "")
			require($this->CORE->CONFIG->SCRIPT['custom_header']);

		echo $this->content;

		// attempt to include custom footer
		if ($this->CORE->CONFIG->SCRIPT['custom_footer'] != "")
			require($this->CORE->CONFIG->SCRIPT['custom_footer']);
	}
	...
	function parseDomains($domain_queries, $template_id)
	{
		...
		foreach($domain_queries as $current_domain)
		{
			...
			$replace = array (
				...
				$this->CORE->DOMAINS->getPunycode($current_domain->getDomain(), $current_domain->getExt()),

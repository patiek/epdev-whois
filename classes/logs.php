...
class EP_Dev_Whois_Logs
{
	...
	function parseTemplate($domain, $extensions, $template, $filename)
	{
		...
		//	NOTICE: Domain punycode is missing and will be added later in loop.
		// +------------------------------
		$replace = array (
				"month" => date("m"),
				"day" => date("d"),
		...
		foreach($extensions as $ext)
		{
			...
			// update replace with punycode
			$replace['domain_puny'] = $this->CORE->DOMAINS->getPunycode($replace['domain'], $ext);
